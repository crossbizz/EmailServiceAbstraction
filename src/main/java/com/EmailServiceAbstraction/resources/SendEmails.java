...
public class SendEmails {
        ...
        block5 : {
            block4 : {
                ...
                try {
                    ...
                    if (clientResponse.getClientResponseStatus() != ClientResponse.Status.OK) break block4;
            ...
            if (clientResponse.getClientResponseStatus() != ClientResponse.Status.OK) break block5;
            return "{'status': 1, 'message':'success'}";
        }
        return "{'status': 1, 'message':'success', 'details':" + clientResponse.toString() + "}";
    }
    ...
    private ClientResponse SendViaMailGun(EmailData emailData) {
        ...
        client.addFilter((ClientFilter)new HTTPBasicAuthFilter("api", MAILGUNKEY));
        ...
        return (ClientResponse)webResource.type("application/x-www-form-urlencoded").post((Class)ClientResponse.class, (Object)formData);
    }
    ...
    private ClientResponse SendViaMandrill(EmailData emailData) {
        Client client = Client.create();
        client.addFilter((ClientFilter)new HTTPBasicAuthFilter("api", MANDRILLKEY));
        ...
        return (ClientResponse)((WebResource.Builder)webResource.type("application/x-www-form-urlencoded").accept(new String[]{"application/json"})).post((Class)ClientResponse.class, (Object)messageData);
    }
}